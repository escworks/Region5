...
namespace tx_r16.Shoebox.Evaluation
{
    /// <summary>
    /// Evaluation Form
    /// </summary>
    public class Evaluation
    {
        ...
        */
        ...
        //Added by VM 1-24-2012
        public DataSet LoadHeaders(int attendee_pk)
        {
            using (SqlCommand SQLCommand = new SqlCommand("[/shoebox/evaluation/form/getHeader]", Connection))//this._DataProvider.GenerateConnection()))
            {
                SQLCommand.CommandType = CommandType.StoredProcedure;
                SQLCommand.Connection.Open();
                SQLCommand.Parameters.AddWithValue("@attendee_pk", attendee_pk);
                try
                {
                    DataSet ds = new DataSet();
                    SqlDataAdapter da = new SqlDataAdapter(SQLCommand);
